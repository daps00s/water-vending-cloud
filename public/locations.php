...
<head>
    ...
    <title><?= htmlspecialchars($pageTitle) ?> - Water Dispenser Management System</title>
    <link rel="stylesheet" href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* Your existing CSS styles here - they look good */
        .modal {
            display: none;
            position: fixed;
            z-index: 1000;
...
<body>
    ...
    const topLocations = [
        ...
        { name: '<?= addslashes($loc['location_name']) ?>', liters: <?= (float)$loc['total_liters'] ?> },
        <?php endforeach; ?>
    ];

    const topMachines = [
        <?php foreach ($top_machines as $machine): ?>
        { description: '<?= addslashes($machine['description']) ?>', location: '<?= addslashes($machine['location_name'] ?? 'Unknown') ?>', liters: <?= (float)$machine['total_liters'] ?> },
        <?php endforeach; ?>
    ];
        ...
        try {
            ...
            allLocations.forEach(location => {
                ...
                const icon = L.divIcon({
                    ...
                    html: `<div style="background-color: ${color}; width: 20px; height: 20px; border-radius: 50%; border: 2px solid #fff; box-shadow: 0 2px 5px rgba(0,0,0,0.3);"></div>`,
                    iconSize: [20, 20],
                    iconAnchor: [10, 10],
                    popupAnchor: [0, -10]
                });
            ...
            // Add click handler for creating new locations
            map.on('click', function(e) {
                ...
                const popupContent = `
                    <div style="text-align: center;">
                        ...
                        Latitude: ${lat}<br>
                        Longitude: ${lng}<br>
                        <button class="btn-primary" onclick="openAddLocationModal(${lat}, ${lng})" style="margin-top: 10px; padding: 5px 10px; font-size: 12px;">Create Location Here</button>
                    </div>
                `;
                L.popup()
                    .setLatLng(e.latlng)
            ...
            document.getElementById('map').innerHTML = '<div style="padding: 20px; text-align: center; color: #666;">Error loading map. Please refresh the page.</div>';
        }
    }
    ...
    function initTopLocationsChart() {
        ...
        try {
            ...
            console.error('Error initializing top locations chart:', error);
        }
    }

    // Initialize top machines chart
    function initTopMachinesChart() {
        const ctx = document.getElementById('topMachinesChart');
        if (!ctx || topMachines.length === 0) {
            console.log('No top machines data available');
            return;
        }
        ...
        try {
            ...
            console.error('Error initializing top machines chart:', error);
        }
    }
